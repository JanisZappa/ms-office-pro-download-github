...
public static class PixelInput
{
    ...
    public static float H2 =>
        Input.GetAxis("HorizontalB") + Input.GetAxis("Mouse X") * .765f;
    ...
    private static void InputUpdate()
    {
        ...
        Cursor.visible = Application.isEditor && Input.GetMouseButton(1);

        //mouseLook = Mathf.Clamp(Input.GetMouseButton(2)? 0 : mouseLook + Input.mouseScrollDelta.y * .2f, -1, 1);
        mouseLook = Mathf.Clamp(mouseLook + Input.GetAxis("Mouse Y") * Time.deltaTime * 3.7272f, -1, 1);
