...
public class SmoothMimic : MonoBehaviour
{
    public Transform goal;
    public Transform smooth;
    private Vector3 pos, smoothPos;
    private Quaternion smmothRot;
...
// Check if rotation is greater than 190 degees == q.w is negative
    ...
    {
        smoothPos = Vector3.Lerp(smoothPos, transform.position, Time.deltaTime * smoothSpeed);
        smmothRot = Quaternion.Slerp(smmothRot, transform.rotation, Time.deltaTime * smoothSpeed);
        
        smooth.position = smoothPos;
        smooth.rotation = smmothRot;
    }
}
