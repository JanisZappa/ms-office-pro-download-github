...
float hack_trowbridgeReitzAnisotropicNDF(float NdotH, float roughness, float anisotropy)
{
    float aspect = sqrt(1.0 - 0.9 * anisotropy);
    float alpha = roughness * roughness;
    ...
    float denominator = PI * roughT * roughB * pow(1 + 1 / (roughB * roughB) + NdotH2, 2);
    return 1 / denominator;
}
