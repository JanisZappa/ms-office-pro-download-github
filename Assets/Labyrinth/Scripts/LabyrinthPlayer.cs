...
public class LabyrinthPlayer  : CharacterController
{
    [Header("Movement")]
    public float speed;
    ...
    private float turn, smoothAngle, lookTurn, look, smoothLook, gravity, jumpH, smoothLookAhead, 
                  lookSpeedMulti = 1, walkTime, stable = 1, crouchOffset;
    ...
    private readonly QuaternionForce wallForce   = new QuaternionForce(200).SetSpeed( 48 * .35f).SetDamp( 6.2f);
    private readonly QuaternionForce hitForce    = new QuaternionForce(200).SetSpeed( 70 * .25f).SetDamp(4.75f);
    private readonly QuaternionForce steerForce  = new QuaternionForce(200).SetSpeed(100 * .75f).SetDamp(9.75f);
    private readonly QuaternionForce lookForce   = new QuaternionForce(200).SetSpeed( 480 * .25f).SetDamp(28.75f);
    private readonly Vector3Force    NeckForce   = new    Vector3Force(200).SetSpeed( 48 * .25f).SetDamp(8.8f);
    private readonly FloatForce      VForce      = new      FloatForce(200).SetSpeed(111 * .4f).SetDamp(11f);
    private readonly FloatForce      CrouchForce = new      FloatForce(200).SetSpeed(45).SetDamp(8.5f);
    private readonly FloatForce  CrouchLookForce = new      FloatForce(200).SetSpeed(34 * .5f).SetDamp(6.5f);
    
    private readonly QuaternionForce headRotForce = new QuaternionForce(200).SetSpeed(240 * .8f * .75f).SetDamp(13.75f * .8f);
    ...
    private float CrouchGoal { get { return crouching? 1.175f : 0; }}
    ...
    private void LateUpdate()
    {
        ...
        bool crouchOrder = LabyrinthInput.Crouch;
        if (crouching)
        {
        ...
        walkTime += Mathf.Max(vel, turnMeasure * .025f) * dt * 2;
        ...
        Quaternion headRot = Quaternion.AngleAxis(VForce.Update(smoothLook, dt) + crouchOffset * .25f + (LabyrinthInput.Sprint? 1.25f : 0), Vector3.right);
        head.localRotation = headRot;
        head.localRotation = LeanAwayRot() * (lookLean * steerLean) * headRot;
        
        
        Quaternion finalRot = headRotForce.Update( head.rotation * EyeSway(vel * .2f), dt);
        
        head.rotation = finalRot;
        
        head.position = headPosForce.Update(head.position + WeightSway(vel), dt);
    ...
    private Quaternion LeanAwayRot()
    {
        ...
        smoothLeanAway = Quaternion.Slerp(smoothLeanAway, Quaternion.Euler(new Vector3(-bounceMV.z, 0, -bounceMV.x) * velMulti), Time.deltaTime * 3.65f * velMulti);
    ...
    {
        float mag = movement.velocity.magnitude / Time.deltaTime * .01f;
        bool moving = mag > .0001f;
        
        float wS       = 6.5f * .8f * (1 + crouch * .3f) * 5f * 4* (LabyrinthInput.Sprint? 1.75f : 1);
        ...
        walkNod  *= .1f * 2.45f * (LabyrinthInput.Sprint? 1.75f : 1);
        walkSway *= .075f * 2.45f * (LabyrinthInput.Sprint? 1.75f : 1);
        
        float turnAxis = LabyrinthInput.H2, walkAxis = LabyrinthInput.V1, strafeAxis = LabyrinthInput.H1;
        
        lookLean = lookForce.Update(Quaternion.AngleAxis(walkSway, Vector3.up) * Quaternion.AngleAxis(turnAxis * -(4 + mag * .5f * lookSpeedMulti) * .5f * .5f * walkNod , Vector3.forward), Time.deltaTime);
    ...
    public void FixedUpdate()
    {
        ...
        float turnAxis   = LabyrinthInput.H2, 
              walkAxis   = LabyrinthInput.V1,
              strafeAxis = LabyrinthInput.H1,
              lookAxis   = LabyrinthInput.V2;
        ...
        smoothMove = Vector3.Lerp(smoothMove, move * speed * lookSpeedMulti * (1f - crouch * .55f) * (LabyrinthInput.Sprint? 1.5f : 1), dt * acceleration);
        
        Vector3 jump = Vector3.up * GetVerticalSpeed(dt) + (movement.isGrounded? Vector3.zero : jumpForward);
        
        movement.Move(smoothMove + jump, speed * 10 * (LabyrinthInput.Sprint? 1.75f : 1), false);
    ...
    {
        bool jumpInput = !crouching && LabyrinthInput.Jump;
