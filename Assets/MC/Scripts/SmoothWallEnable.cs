...
public class SmoothWallEnable : MonoBehaviour
{
    ...
    public static int BatBlockUpdate(Bounds bounds, Vector3 batPos, float radius)
    {
        {
            ...
            int xSteps = Mathf.CeilToInt(range.x / BatBlock.Blocks) + 1;
            int ySteps = Mathf.CeilToInt(range.y / BatBlock.Blocks) + 1;
            int zSteps = Mathf.CeilToInt(range.z / BatBlock.Blocks) + 1;

            for (int x = 0; x < xSteps; x++)
            for (int y = 0; y < ySteps; y++)
            for (int z = 0; z < zSteps; z++)
            {
                Vector3Int checkPos = BatBlock.PosToBlock(min + new Vector3(x, y, z) * BatBlock.Blocks);
                if (dict.ContainsKey(checkPos))
                {
                    GameObject gO = dict[checkPos];
                    gO.layer = MCLayers.SmoothWall;
                    active.Add(gO);
                }
            }
        }
        {
            ...
            int foundCount = 0;
            int fCount = Physics.OverlapSphereNonAlloc(batPos, radius, nonAlloc, MCLayers.Mask_Smooth);
            for (int i = 0; i < fCount; i++)
            {
                GameObject found = nonAlloc[i].gameObject;
    ...
    public static int BatBlockUpdate(Vector3 batPos, float radius)
    {
...

        {
            int aCount = active.Count;
            for (int i = 0; i < aCount; i++)
                active[i].layer = MCLayers.SmoothWall;
            int fCount = Physics.OverlapSphereNonAlloc(batPos, radius, nonAlloc, MCLayers.Mask_Smooth);
            for (int i = 0; i < aCount; i++)
            {
                bool yes = false;
                ...
                gO.layer = yes ? MCLayers.SmoothWall : 0;
            }
